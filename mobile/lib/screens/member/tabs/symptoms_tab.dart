...
class _SymptomsTabState extends State<SymptomsTab> {
  ...
  Future<void> _load() async {
    ...
    final entries = await api.getSymptomEntries(widget.memberId);
    final summary = await api.getSummary(widget.memberId);
    if (mounted) {
      setState(() {
        _entries = entries;
        _summary = summary;
  ...
  Widget build(BuildContext context) {
    if (_entries == null || _summary == null) return const LoadingCenter();
    final api = context.read<AuthProvider>().api;
        ...
        children: [
          ...
          if (_entries!.isEmpty)
            ...
            LedgerTable(
              ...
              rows: [for (final e in _entries!.cast<Map<String, dynamic>>()) _entryRow(e)],
            ),
        ],
      ),
    );
  ...
  LedgerRow _entryRow(Map<String, dynamic> e) {
    ...
    return LedgerRow(
      ...
      sublabel: [date, if (severity != null && severity.isNotEmpty) severity].join(' · '),
